  ...
  if (!resolvedCompanyId) {
    ...
    return (
      <div className="space-y-4">
        <p className="text-sm text-muted-foreground">{t("emptyState.selectCompanyToViewPage", "选择公司以查看此页面。")}</p>
      </div>
    );
  }

  if (!contributions) {
    return <div className="text-sm text-muted-foreground">{t("states.loading", "加载中...")}</div>;
  }
    ...
    if (duplicateMatches.length > 1) {
      return (
        <div className="rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm text-destructive">
          {t("pluginPage.duplicateRoutes", "多个插件声明了相同的路由")} <code>{pluginRoutePath}</code>。{t("pluginPage.duplicateRoutesHint", "在冲突解决之前请使用插件 ID 路由。")}
        </div>
      );
    }
  }
