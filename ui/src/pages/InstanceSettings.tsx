...
export function InstanceSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("instanceSettings.heartbeats", "心跳") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.failedToUpdate", "更新心跳失败。"));
    },
  });
  ...
  const disableAllMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.failedToDisableAll", "禁用所有心跳失败。"));
    },
  });
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("instanceSettings.loading", "正在加载调度器心跳...")}</div>;
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("instanceSettings.failedToLoad", "加载调度器心跳失败。")}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("instanceSettings.schedulerHeartbeats", "调度器心跳")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("instanceSettings.desc", "所有公司中启用了定时器心跳的智能体。")}
        </p>
      </div>

      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span><span className="font-semibold text-foreground">{activeCount}</span> {t("instanceSettings.active", "活跃")}</span>
        <span><span className="font-semibold text-foreground">{disabledCount}</span> {t("instanceSettings.disabled", "已禁用")}</span>
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {grouped.length === 1 ? t("instanceSettings.company", "公司") : t("instanceSettings.companies", "公司")}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              const noun = enabledCount === 1 ? t("instanceSettings.agent", "智能体") : t("instanceSettings.agents", "智能体");
              if (!window.confirm(t("instanceSettings.disableAllConfirm", "禁用所有 {{count}} 个已启用的{{noun}}的定时器心跳？", { count: enabledCount, noun }))) {
                return;
              }
              disableAllMutation.mutate(agents);
            }}
          >
            {disableAllMutation.isPending ? t("instanceSettings.disabling", "禁用中...") : t("instanceSettings.disableAll", "全部禁用")}
          </Button>
        )}
      </div>
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("instanceSettings.noHeartbeats", "没有调度器心跳符合当前条件。")}
        />
      ) : (
                  ...
                  {group.agents.map((agent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {agent.schedulerActive ? t("instanceSettings.on", "开启") : t("instanceSettings.off", "关闭")}
                        </Badge>
                        <Link
                        ...
                        >
                          {agent.lastHeartbeatAt
                            ...
                            : t("instanceSettings.never", "从未")}
                        </span>
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            to={buildAgentHref(agent)}
                            className="text-muted-foreground hover:text-foreground"
                            title={t("instanceSettings.fullAgentConfig", "完整智能体配置")}
                          >
                          ...
                          >
                            {saving ? "..." : agent.heartbeatEnabled ? t("instanceSettings.disableHeartbeat", "禁用定时器心跳") : t("instanceSettings.enableHeartbeat", "启用定时器心跳")}
                          </Button>
                        </span>
                      </div>
                    );
