...
export function Companies() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("companies.title", "公司") }]);
  }, [setBreadcrumbs, t]);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-end">
        <Button size="sm" onClick={() => openOnboarding()}>
          ...
          {t("companies.newCompany", "新建公司")}
        </Button>
      </div>

      <div className="h-6">
        {loading && <p className="text-sm text-muted-foreground">{t("companies.loading", "正在加载公司...")}</p>}
            ...
            >
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      >
                        ...
                        {t("companies.deleteCompany", "删除公司")}
                      </DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </div>
              ...
              <div className="flex items-center gap-3 sm:gap-5 mt-4 text-sm text-muted-foreground flex-wrap">
                <div className="flex items-center gap-1.5">
                  ...
                  <span>
                    {agentCount} {t("companies.agent", "智能体", { count: agentCount })}
                  </span>
                </div>
                <div className="flex items-center gap-1.5">
                  <CircleDot className="h-3.5 w-3.5" />
                  <span>
                    {issueCount} {t("companies.issue", "任务", { count: issueCount })}
                  </span>
                </div>
                <div className="flex items-center gap-1.5 tabular-nums">
                  ...
                  <span>
                    ...
                    {company.budgetMonthlyCents > 0
                      ...
                      : <span className="text-xs ml-1">{t("companies.unlimitedBudget", "无预算上限")}</span>}
                  </span>
                </div>
                <div className="flex items-center gap-1.5 ml-auto">
                  <Calendar className="h-3.5 w-3.5" />
                  <span>{t("companies.created", "创建于")} {relativeTime(company.createdAt)}</span>
                </div>
              </div>
              ...
              {isConfirmingDelete && (
                ...
                >
                  <p className="text-sm text-destructive font-medium">
                    {t("companies.deleteConfirm", "删除此公司及其所有数据？此操作无法撤销。")}
                  </p>
                  <div className="flex items-center gap-2 ml-4 shrink-0">
                    ...
                    <Button
                      ...
                      disabled={deleteMutation.isPending}
                    >
                      {deleteMutation.isPending ? t("companies.deleting", "删除中…") : t("companies.delete", "删除")}
                    </Button>
                  </div>
                </div>
              )}
